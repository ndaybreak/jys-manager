...
function hasPermission(route) {
  ...
  const menus = localStorage.getItem('menus') && localStorage.getItem('menus').split(',') || []
  return menus.indexOf(route.meta.id) >= 0
}
...
router.beforeEach((to, from, next) => {
  ...
  if (getToken()) { // determine if there has token
    ...
    if (to.path === '/login') {
      ...
      if (store.getters.roles.length === 0) { // 判断当前客户是否已拉取完user_info信息
