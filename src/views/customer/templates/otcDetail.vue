<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <!--<el-select clearable style="width: 150px" class="filter-item" v-model="query.template" @change="getList" :placeholder="请选择费率模板">-->
        <!--<el-option v-for="item in otcAdTemplateList" :key="item.id" :label="item.name" :value="item.id">-->
        <!--</el-option>-->
      <!--</el-select>-->
    ...
    <el-table :data="list" border fit highlight-current-row
      ...
      <el-table-column align="center" :label="$t('table.template')" width="150" prop="template_name"></el-table-column>
      <el-table-column align="center" :label="$t('table.coin')" width="80" prop="coin_code"></el-table-column>
      <el-table-column align="center" :label="$t('table.common_min_quantity')" width="140" prop="common_min_quantity"></el-table-column>
      <el-table-column align="center" :label="$t('table.common_max_quantity')" width="140" prop="common_max_quantity"></el-table-column>
      <el-table-column align="center" :label="$t('table.big_min_quantity')" width="140" prop="big_min_quantity"></el-table-column>
      <el-table-column align="center" :label="$t('table.big_max_quantity')" width="140" prop="big_max_quantity"></el-table-column>
      <el-table-column align="center" :label="$t('table.remark')" width="130" prop="info"></el-table-column>
      ...
      <el-table-column v-if="isEdit" align="center" :label="$t('table.actions')" width="180" class-name="small-padding fixed-width">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :rules="rules" :inline="true" ref="dataForm" :model="tempInfo" label-position="left" label-width="170px" style='width: 900px; margin-left:50px;'>
        <el-form-item :label="$t('table.coin')" prop="coin_id">
          <el-select clearable class="filter-item" v-model="tempInfo.coin_id" :placeholder="$t('tip.select')">
            <el-option v-for="item in coinList" :key="item.id" :label="item.coin_code" :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item :label="$t('table.otcTemplate')" prop="template" class="item-right">
          <el-select clearable disabled class="filter-item" v-model="tempInfo.template" :placeholder="$t('tip.select')">
            <el-option v-for="item in otcAdTemplateList" :key="item.id" :label="item.name" :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item :label="$t('table.common_min_quantity')" prop="common_min_quantity">
          <el-input-number v-model="tempInfo.common_min_quantity" :precision="6" :step="0.01"></el-input-number>
        </el-form-item>
        <el-form-item :label="$t('table.common_max_quantity')" prop="common_max_quantity" class="item-right">
          <el-input-number v-model="tempInfo.common_max_quantity" :precision="6" :step="0.01"></el-input-number>
        </el-form-item>
        <el-form-item :label="$t('table.big_min_quantity')" prop="big_min_quantity">
          <el-input-number v-model="tempInfo.big_min_quantity" :precision="6" :step="0.01"></el-input-number>
        </el-form-item>
        <el-form-item :label="$t('table.big_max_quantity')" prop="big_max_quantity" class="item-right">
          <el-input-number v-model="tempInfo.big_max_quantity" :precision="6" :step="0.01"></el-input-number>
        </el-form-item>
...
<script>
  import { otcTempateDetailFetchAll, otcTempateDetailCreate, otcTempateDetailUpdate, otcTempateDetailDelete, otcAdTemplateDropdown, getCoinList } from '@/api/customer'
    ...
    data() {
      return {
        ...
        rules: {
          ...
          common_min_quantity: [{ required: true, message: this.$t('tip.input'), trigger: 'blur' }],
          common_max_quantity: [{ required: true, message: this.$t('tip.input'), trigger: 'blur' }],
          big_min_quantity: [{ required: true, message: this.$t('tip.input'), trigger: 'blur' }],
          big_max_quantity: [{ required: true, message: this.$t('tip.input'), trigger: 'blur' }],
          info: [{ required: false, message: this.$t('tip.input'), trigger: 'blur' }]
        },
        typeList: [{
          id: 1,
          name: intl('map.recharge')
        }, {
          id: 2,
          name: '提现'
        }],
    ...
    methods: {
      getStaticData() {
        otcAdTemplateDropdown().then(result => {
          this.otcAdTemplateList = result.data
        })
      ...
      getList() {
        ...
        otcTempateDetailFetchAll(this.query).then(result => {
      ...
      handleDelete(row) {
        this.$confirm(this.$t('tip.del'), this.$t('tip.warning'), {
          ...
          otcTempateDetailDelete(row.id).then(res => {
            this.$message({
              type: 'success',
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            otcTempateDetailCreate(this.tempInfo).then(data => {
      ...
      updateData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            otcTempateDetailUpdate(this.tempInfo, this.updatingObj).then((res) => {
              this.getList()
              this.dialogFormVisible = false
              this.$notify({
                title: this.$t('tip.successTitle'),
